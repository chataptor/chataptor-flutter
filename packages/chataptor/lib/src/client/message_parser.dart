...
/// Parses a single `message:received` payload into a [Message].
///
...
Message parseIncomingMessage(Map<String, dynamic> payload) {
  // Payload wraps the message object: {message: {...}} — unwrap tolerantly.
  ...
  // Translations arrive as a `translation` sub-object. The top-level
  // `body_translated` is only set by legacy paths, so fall back to the
